...
public class PooledByteBufAllocator extends AbstractByteBufAllocator {
	...
	static {
		int defaultPageSize = SystemPropertyUtil.getInt(
		...
		int defaultMaxOrder = SystemPropertyUtil.getInt(
		...
		DEFAULT_NUM_HEAP_ARENA = Math.max(0, SystemPropertyUtil.getInt(
				"io.netty.allocator.numHeapArenas",
				(int) Math.min(runtime.availableProcessors(), Runtime
						.getRuntime().maxMemory() / defaultChunkSize / 2 / 3)));
		DEFAULT_NUM_DIRECT_ARENA = Math.max(0, SystemPropertyUtil.getInt(
	...
	public PooledByteBufAllocator(boolean preferDirect) {
		this(preferDirect, DEFAULT_NUM_HEAP_ARENA, DEFAULT_NUM_DIRECT_ARENA,
				DEFAULT_PAGE_SIZE, DEFAULT_MAX_ORDER);
	}

	public PooledByteBufAllocator(int nHeapArena, int nDirectArena,
	...
	public PooledByteBufAllocator(boolean preferDirect, int nHeapArena,
		...
		if (nHeapArena < 0) {
			throw new IllegalArgumentException("nHeapArena: " + nHeapArena
					+ " (expected: >= 0)");
		}
		if (nDirectArena < 0) {
			throw new IllegalArgumentException("nDirectArea: " + nDirectArena
					+ " (expected: >= 0)");
		}
		...
		if (nHeapArena > 0) {
			...
			for (int i = 0; i < heapArenas.length; i++) {
				heapArenas[i] = new PoolArena.HeapArena(this, pageSize,
		...
		if (nDirectArena > 0) {
			...
			for (int i = 0; i < directArenas.length; i++) {
				directArenas[i] = new PoolArena.DirectArena(this, pageSize,
						maxOrder, pageShifts, chunkSize);
			}
		} else {
			directArenas = null;
	...
	protected ByteBuf newDirectBuffer(int initialCapacity, int maxCapacity) {
		...
		if (directArena != null) {
			...
			if (PlatformDependent.hasUnsafe()) {
				...
				buf = new UnpooledDirectByteBuf(this, initialCapacity,
						maxCapacity);
			}
		}
