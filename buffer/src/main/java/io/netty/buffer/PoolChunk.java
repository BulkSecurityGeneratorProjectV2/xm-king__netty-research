...
final class PoolChunk<T> {
    ...
    PoolChunk(PoolArena<T> arena, T memory, int pageSize, int maxOrder, int pageShifts, int chunkSize) {
        ...
        for (int i = 0; i <= maxOrder; i ++) {
            ...
            for (int j = 0; j < chunkSizeInPages; j += runSizeInPages) {
            	...
            	//[2-14]:run的大小
            	//[15-31]:run的offset
            	//System.out.println("memoryMapIndex:"+memoryMapIndex+" offset:"+(j)+" runSize:" + runSizeInPages);
            	//此处为什么需要j向左移动17位呢？可能原因是maxorder最大值不能超过14，这里需要以后证实一下
    ...
    long allocate(int normCapacity) {
        ...
        /*
         * 请求大小超过了一个Page，则需要分配若干个Page对象，这些个Page对象组成了Run对象
         * 否则，就直接在Page上分配对象
         */
